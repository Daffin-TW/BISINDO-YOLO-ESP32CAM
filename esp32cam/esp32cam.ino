...
void enterState(SystemState s) {
  ...
  switch (s) {
    ...
    case STATE_BATTERY_CHECK:
      ...
      WiFi.disconnect(true);
      WiFi.mode(WIFI_OFF);
      delay(150); // allow radio to fully shut down
      startBatteryReading();
...
void startStreamServer() {
  ...
  webServer->on("/stream",  HTTP_GET, handleStreamRequest);
...
// Root page – simple HTML with embedded stream and config link
void handleRoot() {
...
void handleStreamRequest() {
  ...
  while (client.connected()) {
    ...
    // Yield to RTOS / allow button polling (check for client disconnect)
